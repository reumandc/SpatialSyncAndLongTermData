#This script makes a figure to explain the basic idea of how interacting Moran effects work. 
#Based on Reuman et al, 2023, Ecography, Figs 1 and S1, but simplified.
#
#Reuman, begun 2023 09 29
...
phase_jitters_1<-2*pi*(2*runif(2)-1)/period
phase_jitters_2<-2*pi*(3*runif(2)-1)/period

vertoffset<-8

envvar_1<-matrix(NA,2,length(tim))
envvar_2<-matrix(NA,2,length(tim))
...
ylimits[2]<-ylimits[2]+.40*diff(ylimits)
...
ep<-1.2
rect(mean(sintro_1_x[,2])+le1-ep,-0.5,mean(sintro_1_x[,2])+le1+ep,vertoffset+4.5,border=NA,col=rgb(255/255,192/255,203/255,.5)) #"pink" is red    255, green  192, blue   203
rect(mean(sinpks_1_x[,3])+le1-ep,-0.5,mean(sinpks_1_x[,3])+le1+ep,vertoffset+4.5,border=NA,col=rgb(135/255,206/255,250/255,.5)) #"lightskyblue" is red    135, green  206, blue   250
#now finish plotting the env vars and proceed
lines(tim,envvar_1[1,],type="l")
lines(tim,envvar_1[2,],type="l")
text(10,vertoffset/2,latex2exp::TeX("population $w_2(t)"),adj=c(0,.5),cex=0.6)
text(10,3*vertoffset/2,latex2exp::TeX("population $w_1(t)"),adj=c(0,.5),cex=0.6)
...
mtext(expression(paste("Scenario 1: Synergistic interactions")),side=3,line=0,cex=1,at=xlimits[1],adj=0)
...
par(xpd=FALSE)
mtext(latex2exp::TeX("$l_{n}$"),side=1,line=0,at=mean(c(sinpks_1_x[1,1],sinpks_2_x[2,1])),cex=0.7,adj=c(.5,.5))
...
ylimits[2]<-ylimits[2]+.4*diff(ylimits)
...
#put rectangles to highlight the synchrony, only done here so other stuff plots over it
ep<-1.2
rect(mean(sintro_2_x[,2])+le2-ep,-0.5,mean(sintro_2_x[,2])+le2+ep,vertoffset+4.5,border=NA,col=rgb(255/255,192/255,203/255,.5)) #"pink" is red    255, green  192, blue   203
rect(mean(sinpks_2_x[,3])+le2-ep,-0.5,mean(sinpks_2_x[,3])+le2+ep,vertoffset+4.5,border=NA,col=rgb(135/255,206/255,250/255,.5)) #"lightskyblue" is red    135, green  206, blue   250
#now finish plotting the env vars and proceed
...
#label ln
par(xpd=NA)
lines(rep(sinpks_2_x[2,1],2),c(vertoffset+1+.5,vertoffset+1+7.5),type="l",lty="solid",col="violet")
par(xpd=FALSE)
...
#panel label and title
mtext(expression(paste("Scenario 2: Antagonistic interactions")),side=3,line=0,cex=1,at=xlimits[1],adj=0)
...
ep<-1.2
rect(mean(sintro_2_x[,2])+le2-ep,-0.5,mean(sintro_2_x[,2])+le2+ep,vertoffset+4.5,border=NA,col=rgb(255/255,192/255,203/255,.5)) #"pink" is red    255, green  192, blue   203
rect(mean(sinpks_2_x[,2])+le2-ep,-0.5,mean(sinpks_2_x[,2])+le2+ep,vertoffset+4.5,border=NA,col=rgb(135/255,206/255,250/255,.5)) #"lightskyblue" is red    135, green  206, blue   250
#now finish plotting the env vars and proceed
lines(tim,envvar_2[1,],type="l",lty="dashed")
lines(tim,envvar_2[2,],type="l",lty="dashed")
text(10,vertoffset/2,latex2exp::TeX("population, $w_2(t)"),adj=c(0,.5),cex=0.6)
...
#label the arrows to show the lags
text(sinpks_2_x[2,2]+2*le2/3,vertoffset+1+.5,latex2exp::TeX("$l_{e2}$"),cex=0.7,adj=c(0.5,-.1))
